...
async function getArticles(recipeUrl){
     try {
        let requestArticles = await fetch(`${recipeUrl}?_limit=16`);
 ...
 function renderPaginationButton(){
      ...
      for (let paginationButton of paginationButtons){
        paginationButton.addEventListener("click",function(e){
                ...
                getArticles(`${recipeUrl}?_limit=16&_page=${dataId}`)
...
//--------------------------
 ...
 function renderData (data){
     container.innerHTML = `
        ${ data.map((item)=>{     
            ...
            let dietarytype = item.dietarytype;
            let preptime = item.preptime;
            let cooktime = item.cooktime;
            return getAsCards(id,author,image,title,readtime,seen,likes,comment,free,mealtype,dietarytype,preptime,cooktime)
        }).join("")}
     `;
 }

 function getAsCards(id,author,image,title,readtime,seen,likes,comment,free,mealtype,dietarytype,preptime,cooktime){
    ...
    <div class="cards">
      ...
      ${free==true?`<div class="free"><div class="freeicon"><p class="freeicontext">Free</p></div></div>`:""} 
      <p class="box_text">
        ${title}
      </p>
  ...
  <div class="hidden">
      <div>
          <pre class="pre">Meal Type:         <span class="boldtext">${mealtype}</span></pre> 
          <pre class="pre">Dietary Type:     <span class="boldtext">${dietarytype}</span></pre>
          <pre class="pre">Prep Time:         <span class="boldtext">${preptime} min</span></pre>
          <pre class="pre">Cook Time:        <span class="boldtext">${cooktime} min</span></pre>
        <hr class="line">
        <p>Community</p>
      </div>
      <div>
 ...
 async function getAllArticles(recipeUrl){
  try {
     let requestArticles = await fetch(recipeUrl);
         if(requestArticles.ok){
              let  getData = await requestArticles.json();
              outData =[...getData] 
              // renderData(getData)
